...
const navigation: NavItem[] = [
  ...
  { name: "Custom Requests", href: "/admin/custom-requests", icon: Heart, badge: 3 },
          ...
          {navigation.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <Icon className={`h-5 w-5 flex-shrink-0 ${isActive ? "text-white" : "text-purple-200"}`} />
                  {!collapsed && (
                    <>
                  ...
                  {collapsed && item.badge && (
                    <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-pink-500 text-xs font-bold border-2 border-purple-900">
                      {item.badge}
                    </span>
                  )}
                  {isActive && (
        ...
        <div className={`flex items-center gap-3 ${collapsed ? "justify-center" : ""}`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-purple-200 truncate">admin@pixieloops.com</p>
            </div>
          )}
        </div>
      </div>
